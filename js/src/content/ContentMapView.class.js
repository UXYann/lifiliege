...
ContentMapView.prototype.populateMapRDC = function() {
	if(this.coordRDC.length != 0){
		...
		jQuery('.landmarks').append('<div class="item mark"data-position="500,500" data-show-at-zoom="0"><div class="lifiPointHolder" id="Lampe0"><div class="lifiPoint">0</div></div></div>');
	}
};
...
// 	console.info('ContentMapView.prototype.populateMap2');
	...
	if(this.coord2.length != 0){
	    for (var i = 1; i < this.coord2.length; i++) {
	        ...
	        jQuery('.landmarks').append('<div class="item mark" data-position="'+this.coord2[ i ]+'" data-show-at-zoom="0"><div class="lifiPointHolder" id="Lampe'+floor2Num+'"><div class="lifiPoint"></div></div></div>');
	    }
    } else {
		jQuery('.landmarks').append('<div class="item mark"data-position="500,500" data-show-at-zoom="0"><div class="lifiPointHolder" id="Lampe0"><div class="lifiPoint">0</div></div></div>');
	}
...
ContentMapView.prototype.addingSpotLights = function() {
    ...
    if(markcount.length =! 0) {
        if(this.lastPositionData != ""){
     		...
     		} else if (this.lastPositionData['mapNumber'] > 27) {
     			//	2ème étage
	     ...
	     // Position de la dernière borne consultée (.currentLifiPoint)
	        ...
	        if(this.positionArray){this.positionArray = this.position.split(',')};
        }else{
        	//nécessaire ?
...
ContentMapView.prototype.onClickFeatures = function(e){
	switch(jQuery(e.currentTarget).attr('id')){
		case 'level1':
	        ...
	        this.legend3.find('.mapIcon').css('background','url("img/cloak.png") 3px 3px no-repeat'); 
	        this.legend4.find('.mapIcon').css('background','url("img/shop.png") 3px 3px no-repeat'); 
	        this.legend5.find('.mapIcon').css('background','url("img/cafe.png") 3px 3px no-repeat'); 
	        this.checkLang();
		break;
		case 'level2':
	        this.zoomContainer.smoothZoom('destroy').css('background-image', 'url(zoom_assets/preloader.gif)').smoothZoom({ 
